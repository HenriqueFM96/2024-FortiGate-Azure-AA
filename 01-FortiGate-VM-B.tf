resource "azurerm_image" "customb" {
...
resource "azurerm_virtual_machine" "custompassivefgtvm" {
  ...
  storage_image_reference {
    id = var.custom ? element(azurerm_image.customb.*.id, 0) : null
  }
  ...
  os_profile {
    ...
    custom_data = templatefile("${var.bootstrap-fgt-vm-B}", {
      ...
      port1_ip        = var.hub-fgt_B-external-ip-address
      port1_mask      = "255.255.255.0"
      port2_ip        = var.hub-fgt_B-internal-ip-address
      port2_mask      = "255.255.255.0"
      passive_peerip  = var.hub-fgt_A-internal-ip-address
